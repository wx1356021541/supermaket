...
class AdminManage:
    ...
    def addOneCashier(self):
        ...
        cash_entry=input("请输入该员工的出生日期(eg: 2020-6-20):").strip()
        try:
    ...
    def addOnePurchaser(self):
        ...
        pur_entry=input("请输入该员工的出生日期(eg: 2020-6-20):").strip()
        try:
    ...
    def addOneCommodity(self):	
        for j in range(1002,1000000):
            i = str(j)
            com_num = i
            com_name = "s"+i
            com_type = "t"+i
            com_size = "件"
            com_price = float(j%100)
            com_mdate = "2020-6-20"
            com_edate = "2021-6-20"
            com_quantity = j%1000
            try:
